...
function getAgeAtInauguration(presidentsArr) {
  return presidents.map(function(presidents) {
    return Object.assign({}, presidents, { ageAtInauguration: presidents.inaugurationYear - presidents.birthYear });
});
...
function getPresidentsBornAfter(presidentsArr, year) {
  return presidentsArr.filter(presidents => presidents.birthYear > 1945)
}
...
function countRepublicanPresidents(presidentsArr) {
  return presidentsArr.reduce((count, presidents)=> (presidents.party == "Republican" ? count + 1 : count), 0)
}
